    ...
    test("ensuring") {
      ...
      test("acquireRelease") {
        ...
        for {
          ...
          value <- latch.await *> Live.live(fiber.join.disconnect.timeout(1.second)).as(Some(42))
    ...
    suite("Backpressuring") {
      ...
      test("zipPar") {
        ...
        test("disconnect") {
          Live.live(for {
            ref <- Ref.make(true)
            _   <- (ZIO.sleep(5.seconds) *> ref.set(false)).disconnect.timeout(10.millis)
            v   <- ref.get
          } yield assertTrue(v))
        }
    }
  ...
  def spec =
    suite("BasicDerived") {
      ...
      test("ensuring") {
         ...
         */
        test("acquireRelease") {
          def acquireReleaseWith[R, E, A, B](acquire: ZIO[R, E, A])(release: A => UIO[Any])
    ...
    suite("UninterruptibleMask") {
      ...
      test("overly interruptible") {
        ...
        for {
          ...
          fiber   <- ZIO.uninterruptibleMask(restore => for {
                        child <- doWork(queue, worker(database)).repeatN(4).fork
                        _ <- restore(child.join)
                        } yield child)
