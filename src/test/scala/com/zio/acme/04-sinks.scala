    ...
    suite("Constructors") {
      ...
      test("count") {
        ...
        test("foldLeft") {
          ...
          for {
            value <- stream.run(ZSink.foldLeft(1)((s, v) => s*v))
  ...
  def spec =
    suite("Operators") {
      ...
      test("map") {
         ...
         */
        test("zipPar") {
          def zippedSink: ZSink[Any, Nothing, Int, Nothing, (Long, Int)] = ZSink.count zipPar ZSink.sum[Int]
