    ...
    suite("Introduction") {
      ...
      test(">>>") {
        ...
        test("transduce") {
          ...
          for {
            values <- stream.transduce(ZSink.collectAllN[Int](10)).runCollect
          } yield assertTrue(values.length == 10)
        }
    }
}

object Constructors05 extends ZIOSpecDefault {
  def spec =
    suite("Constructors") {
      /**
       * EXERCISE
      ...
      test("splitLines") {
        ...
        for {
          values <- (stream >>> ZPipeline.splitOn("\n")).runCollect
         ...
         */
        test("fromFunction") {
          def parseInt: ZPipeline[Any, Nothing, String, Int] = ZPipeline.map(_.toInt)
...
object Operators05 extends ZIOSpecDefault {
  def spec =
    suite("Operators") {
      /**
       * EXERCISE
      ...
      test(">>>") {
        ...
        def composed : ZPipeline[Any, CharacterCodingException, String, String] =
            ZPipeline.utf8Encode >>> ZPipeline.utf8Decode
        ...
        } yield assertTrue(values.toList.foldLeft("")((a, b) => a ++ b) == chunk.toList.foldLeft("")((a, b) => a ++ b))
      }
    }
}
