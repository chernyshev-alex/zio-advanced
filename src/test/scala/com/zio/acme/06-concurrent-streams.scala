...
import zio.{Chunk, Promise, Ref, Schedule, ZIO, durationInt}
  ...
  def spec =
    suite("ConcurrentOps") {
      ...
      test("timeout") {
        ...
        Live.live(for {
          size <- stream.runHead.timeout(10.milliseconds)
        ...
        test("flatMapPar") {
          ...
          for {
            ages <- stream.flatMapPar(2)(lookupAge(_)).runCollect
        ...
        test("broadcast") {
          ...
          for {
            ref <- Ref.make(0)
            _ <- ZIO.scoped {
              stream.broadcast(10, 100)
                .flatMap(streams => ZIO.foreach(streams)(stream => consumer(ref, stream)))
                .map(_ => ())
            }
            v <- ref.get
        ...
        test("aggregateAsync(foldUntil(...))") {
          ...
          def sink: ZSink[Any, Nothing, Int, Int, Int] = ZSink.foldUntil(0, 2)(_ + _)

          for {
            values <- stream.aggregateAsync(sink).take(5).runCollect
        ...
        test("aggregateAsync(foldWeighted(...))") {
          ...
          def sink: ZSink[Any, Nothing, Int, Int, Chunk[Int]] =
            ZSink.foldWeighted[Int, Chunk[Int]](Chunk.empty[Int])((ch, _) => ch.length, 2)((ch, c) => ch :+ c)
