  ...
  def spec = suite("RuntimeSpec") {
    ...
    test("isFatal") {
      ...
      Runtime.default.unsafe.run {
        ZIO.attempt(throw ioException)
          .provide(Runtime.setReportFatal(captureFatal), Runtime.addFatal(classOf[java.io.IOException]))
      }
    ...
    test("supervisor") {
      ...
      val effect =
        for {
          ...
          _ <- if (count <= 0)
            ZIO.dieMessage("Not time to live!")
      ...
      lazy val supervisor: Supervisor[Unit] = new Supervisor[Unit] {
        ...
        def onStart[R, E, A](environment: ZEnvironment[R],
                              effect: ZIO[R, E, A],
      ...
      // TODO failed
      Runtime.default.unsafe.run {
          effect.supervised(supervisor).provide(Runtime.enableOpSupervision)
      }
